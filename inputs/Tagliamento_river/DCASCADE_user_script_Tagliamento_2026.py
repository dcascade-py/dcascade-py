...
#---River shape files
path_river_network = Path('../../04-Tagliamento_case/Inputs/')
...
#---Discharge files
path_q = Path('../../04-Tagliamento_case/Inputs/')
...
sed_range = [-8, -1]  # range of sediment sizes - in Krumbein phi (φ) scale (classes from coarse to fine – e.g., -9.5, -8.5, -7.5 … 5.5, 6.5).
n_classes = 12        # number of classes

#---Timescale
timescale = 1460 #1460#1460 # days
...
# CS_curves = None             # Dictionnary. The keys are the reach FromN, and the elements are dictionnaries of numbered 2D array of cross section data. 
                               # It is possible to have many cross sections per reach. 
                               # First row is lateral distance. Second row is bed elevation. 


############### init CS curves
CS_path = Path('../../04-Tagliamento_case/Inputs/Tagliamento_CS_pinz/')
CS_csv1 = pd.read_csv(CS_path / "165.csv")
CS_csv2 = pd.read_csv(CS_path / "166.csv")
CS_csv3 = pd.read_csv(CS_path / "168.csv")
CS_csv4 = pd.read_csv(CS_path / "169.csv")
CS_csv5 = pd.read_csv(CS_path / "170.csv")

CS_curves = {} 

Pinz_FromN = 38
CS_curves[Pinz_FromN] = {}
CS_curves[Pinz_FromN][1] = CS_csv1[['Y', 'Z']].to_numpy().T
CS_curves[Pinz_FromN][2] = CS_csv2[['Y', 'Z']].to_numpy().T
CS_curves[Pinz_FromN][3] = CS_csv3[['Y', 'Z']].to_numpy().T
CS_curves[Pinz_FromN][4] = CS_csv4[['Y', 'Z']].to_numpy().T
CS_curves[Pinz_FromN][5] = CS_csv5[['Y', 'Z']].to_numpy().T
