...
for sc in scenarios:
    ...
    d50_data = np.nan_to_num(data_output[output_d50], nan=0.0)
    ...
    ax.set_title(f"{scenario_label} | {output_d50} | All timesteps | Reaches 1 to {max_reach}", fontsize=16)
    ...
    fig.savefig(
        os.path.join(figure_folder, f"{scenario_label}_{safe_name(output_d50)}_all_timesteps_1_51.png"),
        dpi=300
    )
    ...
    d50_all_df.to_csv(
        os.path.join(csv_folder, f"{scenario_label}_{safe_name(output_d50)}_all_timesteps_1_51.csv"),
        index=False
    )
