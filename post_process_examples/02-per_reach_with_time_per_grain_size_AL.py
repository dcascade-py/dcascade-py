...
output_name = 'e_max_vol_gs'   # Output available in pickle file
...
for i in range(my_data.shape[1]): #Loop over each reach
    ...
    for GS in range(n_class - 1, -1, -1):    
        ax.plot(my_data[:, reach_FromN - 1, GS], linewidth = 1, label = f'd = {dmi[GS]:.2g} mm', color=cmap(GS)) 

       
    ax.set_title('FromN: '+str(reach_FromN), fontsize = 16)
    ax.set_xlabel('Time', fontsize = 13)
    ax.set_ylabel('Vout / V_al, per GS',fontsize = 10)
