...
def GSD_std(Fi_r_reach, dmi_finer):
    ...
    for i in range(0, np.size(D_values)):
        ...
        D_changes[i] = (D_values[i] - Perc_finer[indx + 1]) / (
            Perc_finer[indx] - Perc_finer[indx + 1]) * (
                dmi_finer[indx] - dmi_finer[indx + 1]) + dmi_finer[indx + 1]
...
def Molinas_rates(Fi_r_reach, h, v, Slope, dmi_finer, D50_finer):
    """
     MOLINAS-rates returns the Molinas coefficient of fractional transport rates Pci,
     to be multiplied by the total sediment load to split it into different classes.
    ...
    frac1 = Fi_r_reach.squeeze() * ((dmi_finer / Dn)**alpha + Zeta * (dmi_finer / Dn)**beta)
...
def Wilcock_Crowe_tr_cap(Fi_r_reach, psi, D50, Slope, Wac, h):
    ...
    Fr_s = np.sum((psi > -1) * Fi_r_reach.transpose())
...
def Engelund_Hansen_tr_cap(Fi_r_reach, psi, D50, Slope, Wac, v, h):
    ...
    # m3/s (%formula from the original cascade paper):
    qEH_dim = qEH * np.sqrt((rho_s / rho_w - 1) * g * (D50)**3)
...
def Wong_Parker_tr_cap(Fi_r_reach, psi, D50, Slope, Wac, v, h):
    ...
    # dimensionful transport capacity m3/s
    # m3/s (%formula from the original cascade paper):
    qWP_dim = qWP * np.sqrt((rho_s / rho_w - 1) * g * (D50)**3)
...
def Yang_tr_cap(Fi_r_reach, psi, D50, Slope, Q, v, h):
    ...
    % formulas. (1989).,
    % see also: Modern Water Resources Engineering:
    % https://books.google.com/books?id=9rW9BAAAQBAJ&pg=PA347&dq=yang+sediment+transport+1973&
    % hl=de&sa=X&ved=0ahUKEwiYtKr72_bXAhVH2mMKHZwsCdQQ6AEILTAB#v=onepage&q=yang%20sediment%20
    % transport%201973&f=false
    """
    ...
    '''
    %   1) settling velocity for grains - Darby, S; Shafaie, A. Fall Velocity of Sediment
    %      Particles. (1933)
    %
    ...
    %            w = 0.51*nu/D50*(D50^3*g*R/nu^2)^0.963;
    %       else
    %            % EQ. 4: http://www.wseas.us/e-library/conferences/2009/cambridge/WHH/WHH06.pdf
    %            w = 0.51*nu/D50*(D50^3*g*R/nu^2)^0.553;
    %       end
    '''
