...
# DataFrame to store the X values used for each model run
X_used_df = pd.DataFrame(columns=['Model_Run', 'Wac_Multiplier', 'Slope_Multiplier'])


# Iterate through N samples and modify each row in 'Wac' and 'Slope' with different values from X
for i in range(N):
    ReachData_modified = ReachData.copy() 
    
    # Element-wise multiplication for each row in 'Wac' with different values in X[:, 0]
    Wac_random_values = np.random.choice(X[:, 0], len(ReachData_modified))
    ReachData_modified['Wac'] = ReachData_modified['Wac'] * Wac_random_values
    
    # Element-wise multiplication for each row in 'Slope' with different values in X[:, 1]
    Slope_random_values = np.random.choice(X[:, 1], len(ReachData_modified))
    ...
    # Save the unique X values used for 'Wac' and 'Slope' in this model run
    X_used_df = X_used_df.append({'Model_Run': i+1, 
                                  'Wac_Multiplier': Wac_random_values[0],  # Saving one value if random choice for all rows was the same
                                  'Slope_Multiplier': Slope_random_values[0]}, 
                                 ignore_index=True)

# Save the X values DataFrame to a CSV file
X_used_df.to_csv(path_ReachData + "X_values_used.csv", index=False)
