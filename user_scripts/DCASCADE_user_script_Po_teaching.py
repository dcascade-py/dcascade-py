...
GSD_perc_list = [-0.2, -0.1, 0, 0.1, 0.2]
...
for GSD_perc in GSD_perc_list:
    for width_input_name, width_output_name in zip(width_input_name_list, width_output_name_list):
        for tl, name in zip(transport_law, name_list):
            ...
            #-------River shape files 
            path_river_network = Path('../inputs/Input_Po_case_PGS_2025/shp_slope_DEM_2021_smoothed/')
            ...
            #--------Path to the output folder
            path_results = Path("../cascade_results/")
            name_file = path_results / f"{name}_{GSD_perc}.p"
            name_file_ext = path_results / f"{name}_{GSD_perc}_ext.p"
            ...
            #---Timescale 
            timescale = 20       # 5843 days 1095, 1825
