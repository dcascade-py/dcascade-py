...
i = 17
Y = pd.to_numeric(df_Y[f"Reach {i}"]).values  # Use corresponding column from df_mob
#Reach 21 = 22
Y = Y.reshape(-1, 1)


i = 17
X = np.column_stack(((df1.iloc[:, i-1]).values, (df2.iloc[:, i-1]).values))  # Use corresponding column from df_mob
#Reach 21 = 22
