...
Note 03/23 The variables divided by sediment class are not implemented yet
(commented lines are the 'work in progress stage'..)
...
def dynamic_plot(data_output, ReachData, psi, **kwargs):
    ...
    #     np.unique(np.percentile(data_output['Transported + deposited sed in the reach [m^3/s]']\
    #     [np.nonzero(data_output['Transported + deposited sed in the reach [m^3/s]'])], \
    #     np.arange(0,100,i_class)))
    ...
    slider = Slider(axSlider, 'Time step', valmin=1, valmax=len(plotvariable) - 1,
                    valfmt='%0.0f', color='grey')
    button1 = Button(axpos1, '<', color='w', hovercolor='b')
    button2 = Button(axpos2, '>', color='w', hovercolor='b')

    def value_update(t):    
        time_step = round(t)
        plot_network_dyn(ReachData, plotvariable, time=time_step, 
                         CClass=np.unique(np.around(cClass[plot_class], 5)), ax=ax, fig=fig)
