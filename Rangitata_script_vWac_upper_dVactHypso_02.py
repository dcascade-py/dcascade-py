...
from DCASCADE_loop_vWac_dVact import DCASCADE_main, ReachData
...
path_results = Path("./Results/Rev2_HypsoSolv/ActHyps_Q220_2024_icFs02/")
...
# Define the initial deposit layer per each reach in [m3/m] ccJR this is now a DEPTH which is multiplied by Wacmax later
...
deposit = reach_data.deposit * reach_data.length * reach_data.maxwac
...
for NR in range(nrepeats):    
    ...
    #new Qbi_dep_in. Tried tot_sed_class but    that is different from the Fi_active. Until I save and read stratigraphy, let's
    ...
    print('Fix this as its using Fi_r_ac and I want saved Fi_stratigraphy')
    nsave_dep_time = data_output['V_dep_sum'].shape[0]-1
    FiFinal_Layers = data_output['Qbi_FiLayers'][-1] # final time index
    FReach_Layers = data_output['Qbi_FiLayers'][:,2,:,:] # one reach all time index
    for n in range(reach_data.n_reaches):
