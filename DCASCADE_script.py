...
- deposit = initial deposit layer expressed in m3/m - this value will be then multiplied by the reach width and length 
...
path_river_network = 'C:\\Users\\user1\\OneDrive - Università degli Studi di Padova\Documenti\\Postdoc_Padova\\cascade\\matlab\\DCASCADE\\version_latest\\Input_data\\'
#name_river_network = 'River_Network.shp' # shapefile of river network (for now calculated in matlab)
name_river_network = 'file.shp'
path_q = 'C:\\Users\\user1\\OneDrive - Università degli Studi di Padova\\Documenti\\Postdoc_Padova\\cascade\\python\\DCASCADE_toolbox_v2\\Input\\Bega_river\\'
name_q = 'Q_Bega.csv' # csv file that specifies the water flows as a (nxm) matrix, where n = number of time steps; m = number of reaches (equal to the one specified in the river network)
...
sed_range = [-7.5 , 2] #range of sediment sizes considered in the model - in log scale where each number is the average diameter of that sediment class (classes from coarse to fine – e.g., -9.5, -8.5, -7.5 … 5.5, 6.5). 
...
# define the water discharge 
#Q = pd.read_csv(path_q + name_q , header = None, sep=';') # read from external csv file
q = np.matlib.repmat(ReachData['Q'].to_numpy(), timescale, 1) # read from ReachData 
Q = pd.DataFrame(data = q)*0.05
