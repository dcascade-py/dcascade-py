...
def DCASCADE_main(ReachData , Network , Q , Qbi_input, Qbi_dep_in, timescale, psi, roundpar):
    ...
    V_dep = np.zeros((timescale-1, n_reaches ))  # EB : last time step would be equal to 0 - delete to avoid confusion 
    ...
    tot_sed = V_dep + np.sum(QB_tr, axis = 1) 
    ...
    for t in np.arange(len(Qbi_mob_class)):
        mob_sed_temp.append(np.vstack(Qbi_mob_class[t]))
        Fi_mob_t = mob_sed_temp[t]/ (np.sum(mob_sed_temp[t],axis = 1).reshape(-1,1))
    ...
    for t in np.arange(len(Qbi_dep_class)):
        dep_sed_temp.append(np.vstack(Qbi_dep_class[t]))
        Fi_dep_t = dep_sed_temp[t]/ (np.sum(dep_sed_temp[t],axis = 1).reshape(-1,1))
    ...
    for c in range(n_classes): 
        for t in range(timescale-1): 
            q_t = Qbi_dep[t]
            q_tt = Qbi_tr[t][:,:,c]
            tot_sed_class[c][t,:] = np.array([np.sum(item[:,c+1], axis = 0) for item in q_t]) + np.sum(q_tt, axis = 0)
    ...
    data_output = { 'Channel Width [m]': np.repeat(np.array(ReachData['Wac']).reshape(1,-1),len(Qbi_dep), axis = 0), 
                   ...
                   'Reach Slope':Slope, 'D50 deposited layer [mm]' :D50_dep, 'D50 mobilised layer [mm]':D50_mob,
                   'D50 active layer [mm]' :D50_AL,  
                   'Daily trasport capacity [m^3/day]': tr_cap_sum, 'Deposited volume[m^3]': V_dep, 
    ...
    extended_output = { 'Qbi_tr': Qbi_tr,  
                   ...
                   'Qbi_dep':Qbi_dep, 'Fi_r_ac' :Fi_r_act , 'D50_AL' :  D50_AL, 
                   'Node_el' : Node_el }
